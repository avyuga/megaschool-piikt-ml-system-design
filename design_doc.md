...
<img src="sys_arch.png" width="800" >
...
* Вся система будет развернута в контейнере Docker, чтобы минимизировать влияние сторонних процессов, оркестрация процесса будет проводится с помощью Kubernetes.
...
Данные из внутренней системы ресторана попадают в пайплайн, внутри которого проходит их обработка. В результате этой обработки в системе появляется дополнительное поле, указывающее на работника, который должен выполнить задачу для оптимизации общего времени выполнения. При этом такой подход удачно встраивается в имеющуюся архитектуру и е нарушит работу ресторана в случае технических проблем.
